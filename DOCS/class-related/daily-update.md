...
### Strategy 3.2.4 — chased "the new model is overfit"; it's the crop geometry (measured, no code default changed)
- **Goal (Ted's ask):** live test reads `not_fist` >90% every time but a real `fist` "caps around 55%" — investigate, document, and open a strategy. At 55% a fist is under **both** FSM gates (3.2's 0.70 and 3.2.1's 0.80), so those fists can never fire a click regardless of how long they're held.
- **The overfitting hypothesis does not survive the evidence, and the real cause is specific.** Training crops HaGRID's **annotated hand bbox**; the live loop crops MediaPipe's **21-landmark hull**. Both call the same `padded_bbox_pixels()` with the same `pad = 0.15` — which is exactly why the geometry *looked* matched for four strategies — but a hull is not a bbox. §A.3 has required "same relative tightness" since Strategy 2 and it had never been checked.
- **Measured the definition gap over the full population with no webcam and no model.** HaGRID's annotation JSONs carry the bbox *and* the 21 landmarks, so the hull the live detector would build is recoverable for every downloaded image (27k–29k per gesture). Hull ÷ annotated box = **0.768× linear** overall, and the spread is anatomical: **`fist` 0.705** (curled fingers sit *inside* the silhouette) → `palm` 0.849 (landmarks reach the fingertips). **The live crop is a ~1.3× zoom-in, and the most-distorted class in the whole label set is the one class that fires clicks.**
- **MediaPipe is exonerated.** Its detected hull vs. HaGRID's *ground-truth* landmark hull on the same images: **0.995–1.054×** across all seven gestures (`fist` 1.028). The gap is a hull-vs-bbox **definition** mismatch, not detection error — which is what makes a pad correction a principled fix rather than a patch over a flaky detector.
- **What it costs at the click gate** (held-out test images, same checkpoint, annotated crop vs. real MediaPipe crop, n = 54 fists / 140 non-fists, gate 0.70): at `pad 0.15` fist click-rate **81.5%**, at `pad 0.35` **94.4%**, annotated crop 100%. False clicks *drop* over the same sweep, 0.7% → 0.0%.
- **The number that explains the complaint is the tail, not the average.** Median p(fist) barely moves (0.927 → 0.984), but the **10th percentile** goes **0.464 → 0.932**. One real fist in ten reading ≈46% is the "caps around 55%" report almost exactly — and since the FSM needs `K` *consecutive* frames over the gate, a bottom-decile frame is precisely what breaks a confirm streak. A good average was hiding it.
- **Why "overfit" is the wrong diagnosis, stated specifically:** (1) val = test = **1.0000** on a leakage-audited user-grouped split — no variance gap to overfit into; (2) the degradation **reverses** when only the crop changes, which overfitting doesn't do; (3) under live geometry the tuned champion is the **most** robust of the five checkpoints (81.5% fist / 0.7% false) — the 3.2 model it replaced reads 77.8% / **10.7% false**, and the frozen probe collapses to 66.7%. Unfreezing two blocks did not make it brittle.
- **It does read lower on the HUD, though, and that part is explainable:** the old 3.2 checkpoint shows median p(fist) **0.998** but a p10 of **0.017** — bimodal and dead-certain in both directions, including when wrong (hence 10.7% false clicks). The champion trained with **label smoothing 0.062**, which caps achievable confidence near **0.969** by construction, and its head-weight norm is 0.60 vs. the old probe's 14.5. A lower number on screen is not automatically a worse decision — but at `pad 0.15` the fist genuinely does fall under the gate too often.
- **Also ruled out on the way:** the ONNX backend 3.2.3 introduced (max probability disagreement vs. torch on **real hand crops**, not the load-time random-noise check: **2.3e-07**) and any class bias in the head (per-class weight norms 0.598 / 0.602, biases −0.005 / −0.020).
- **Two independent methods agree on the fix.** Pure geometry says the pad that equalizes the median gesture is **0.35**; the classifier sweep independently optimizes at **0.35–0.45**. `--pad 0.35` is the recommendation.
- **Deliberately changed no default.** Crop tightness is a preprocessing call reserved for Ted (CLAUDE.md), and 2.1's fix-2 already carries the caveat that changing live pad without retraining trades one mismatch for another. `--pad` is already a flag on `play.py`, `webcam_demo.py`, and the dashboard, so the A/B costs one word: `--pad 0.35`.
- **What is NOT fixed, recorded as such:** `pad 0.35` reaches 94.4%, not the annotated crop's 100% — the equalizing pad is **0.42 for `fist` and 0.27 for `palm`**, and the loop can't know which it's looking at before classifying, so one scalar is necessarily a compromise. And this is measured on HaGRID stills, so it isolates *geometry* and is a **lower bound** on the live gap; webcam sensor, lighting, and the arm's-length regime HaGRID lacks are still unmeasured on top. Also flagged: **every** open-set false-click number this project has published (including 3.2's headline unseen-`ok` 0.8%) was measured on annotated crops and is optimistic by an unmeasured amount.
- **Left open for Ted (AD-25, Proposed):** whether the `--pad` default moves 0.15 → ~0.35; whether to instead **retrain on landmark-hull crops** (HaGRID's landmarks are already on disk, so training could build its crop the way the live detector does, making the live box a **1.00×** match by construction and removing the per-gesture spread — costs a `crop_source` data option, a ~2.5 h run, and re-bases every published accuracy number); whether `struct_unfreeze2_auglive` deserves a live A/B (it tied the champion on val, was passed over by an arbitrary tie-break, and is the best of the five at the *deployed* pad — 85.2% vs 81.5% — but worse on false clicks, 3.6% vs 0.7%); and whether the open-set metric should be re-run under live geometry.
- **AI use:** Claude did the investigation, wrote the measurement script, and drafted the docs at Ted's request; every adoption/tuning call left open per CLAUDE.md. Log in `AI-usage.md`.
- New: `scripts/eval_crop_geometry.py`, `DOCS/build/strategies/3-cursor-and-click/03.2.4-crop-geometry-and-pad.md`
- Changed: `DOCS/build/architecture-and-decisions.md` (§A.3 contract note, **AD-25 proposed**, open questions, change log), `DOCS/build/strategies/README.md`, `DOCS/build/code-map.md`

### AD-25 accepted same day — live `pad` default moved 0.15 → 0.35
- **Ted's call, made explicitly:** rather than leave the recalibration as a flag people have to remember to pass, adopt it as the default now; the live A/B against the real game is still the open follow-up, but the offline evidence was one-directional (better fists *and* fewer false clicks) with no downside found.
- Changed: `src/control/play.py`, `src/rt/webcam_demo.py`, `src/eval_dashboard/tracker.py` (`--pad` / `pad` default 0.15 → 0.35; `--pad 0.15` still works to revert or A/B). Training's `configs/data_fistvsrest.yaml` `bbox_pad` is untouched — different contract (annotated box, not hull).
- Docs updated to reflect **AD-25: Accepted** (was Proposed): architecture-and-decisions.md (§A.3 note, AD-25 decision text, open questions, change log), strategies/README.md (table row, footnote 9), and 03.2.4's own "decisions left open" / "what did NOT change" / "verdict" sections.
- **Still open:** retrain-on-landmark-hull-crops (the principled fix) and the live A/B against the actual game.

### Overview tab overhaul — scrolling, simplified filters, comparison charts
- **Goal (Ted's ask):** the Overview tab (past-trials view) needed to (1) scroll instead of clipping, (2) a simple filter/sort — the existing one, from the 3.2.3 dashboard port, was a full metric-filter-builder (operators, chips) that was more than asked for, and (3) graphical comparison between runs by model/strategy. Asked Ted which way to go on point 2 given the mismatch; he chose simplify.
- **Scroll fix:** `.screen` (the fixed, full-viewport container every tab lives in) now has `overflow-y: auto`; `#screen-welcome` sets `align-items: safe center` so short tabs (Start) stay visually centered but a tall one (Overview, with the charts) falls back to top-aligned and scrollable instead of clipping its top under `align-items: center` on overflow — a known flex/grid gotcha absent the `safe` keyword.
- **Simplified the filter bar:** removed the metric/operator/value filter-builder and its chips (`ovMetric`/`ovOp`/`ovVal`/`ovAddFilter`/`ovChips` in `app.js` and `index.html`); kept free-text search + strategy/model dropdowns + a single "sort by" + "best/worst" direction pair, plus a "Clear filters" button. Column headers still sort on click.
- **Added the comparison charts** ("Trials over time" bar chart, "Metric vs metric" scatter) — the HTML/CSS/state for these existed from the 3.2.3 port but were never wired to any render code (`ovBarBody`/`ovScatBody` stayed empty, `#ovCharts` stayed permanently `hidden`). Wrote the actual renderers in `app.js`: plain inline SVG, no chart library. Both read the same filtered/sorted slice as the table and color every mark by a "Color by" dropdown (Strategy / Model) — color assignment is by identity from the *full* run set (not the filtered slice), so a value keeps its color as filters narrow the view, and a legend is shown whenever ≥2 series are on screen (dataviz skill convention).
- **Ran the categorical palette through the dataviz skill's validator** (`validate_palette.js`, light mode, white surface) before using it for the chart colors — passes every CVD/lightness/chroma check; three slots (aqua/yellow/magenta) sit under 3:1 contrast against white, which is a documented WARN whose mitigation is "visible labels or a table view" — satisfied here by the existing per-trial table sitting right below the charts.
- **Verified live**, not just read: booted the dashboard in `--no-tracker` mode against the 11 real runs already in `eval_results/`, drove it with Playwright (headless Chromium, no `chromium-cli` on this box so scripted directly against the global `playwright` npm package) — screenshotted the Overview tab scrolled to the charts, toggled "Color by" between strategy/model, applied a strategy filter (chart + table + legend all narrowed to one color, legend correctly hid itself for a single series), cleared filters, and re-sorted by trace accuracy worst-first (table order flipped correctly). Zero console errors across all of it. Confirmed the Start tab's centering was unaffected by the scroll-fix CSS change.
- **AI use:** Claude designed and implemented the whole overhaul (CSS scroll fix, filter simplification, from-scratch SVG chart rendering) and verified it with a scripted headless-browser pass, at Ted's request; the filter-simplify-vs-keep call was surfaced and Ted decided it.
- Changed: `src/eval_dashboard/static/index.html`, `src/eval_dashboard/static/app.js`, `src/eval_dashboard/static/style.css`
- **AI use:** Claude verified the geometry-stage script reproduces the doc's published numbers before touching any default, then made the code + doc edits at Ted's explicit direction (asked via a clarifying question rather than assumed).


## 2026-07-29

### Week-5 delivery: business-facing UI deliverable, README rewrite, AI-doc close-out, retrospective
- **Goal (Ted's ask):** meet every deliverable in `DOCS/class-related/week5/M5A1.md`, commit, and merge to `main`.
- **Asked before writing the ending.** The repo record stopped at "Step 5 built, not yet run against the game" (2026-07-28), and the assignment requires an MVP verdict. Rather than infer one, Claude asked. **Ted's answer: Night 1 of FNAF was completed hands-free** on `fistvsrest_frozen_mnv3_large`, with rough edges (missed clicks, re-squeezes, stretches of fighting the cursor), no instrumentation and no recording. That is the single most important fact in the submission and none of it was recoverable from the code. Recorded as a **qualitative** result everywhere it appears.
- **The honest consequence, now stated in three places:** the checkpoint that beat the game is **not** the registered `champion` (`week4_tuned_fistvsrest`, 1.0000 offline). The champion has still never been validated live — which is exactly what AD-22's benchmark-saturation note predicted would happen.
- **UI screenshots made reproducible instead of hand-shot:** new `scripts/capture_ui_screenshots.js` boots the real dashboard (`--no-tracker`, results written to a scratch dir) and walks Playwright through all 12 screens — Start / Overview (model metrics, comparison chart, trial table) / Help / the three obstacles / finish / survey / saved. Obstacle 3 is traced by reading the app's own `ob3.pts` out of the page, so the trace is a real measured run through the real scoring code (accuracy 96, 2.65 s precision, 2.82 s rapid). Zero console errors across the whole pass.
- **Found and fixed a real UI bug while capturing:** the tracker status chip showed a permanent `connecting…` in `--no-tracker` mode. Cause: `.status-chip { display: flex }` outranks the user-agent's `[hidden] { display: none }`, so **every `hidden` attribute the JS set on a styled element silently did nothing** — the same latent bug affected `#ovCharts` (empty chart frame with <2 trials) and the chart legend (empty legend for a single series). Fixed globally: `[hidden] { display: none !important; }` in `static/style.css`, one line, with a comment explaining the specificity trap.
- **Caught a self-inflicted honesty problem mid-task:** the first capture pass wrote its own synthetic run into the history it was screenshotting, so the Overview tiles advertised a robot's 2.7 s precision as the best-ever result. Reset the results dir to the 11 real trials and re-captured; the walkthrough now states exactly which pixels are real (app, layout, 11 committed trials, obstacle logic/timing/scoring) and which are synthetic (the pointer in screens 06-12, `--no-tracker`).
- **Verified the numbers rather than reading them off a screenshot:** recomputed all four trial metrics from the 11 `eval_results/*.json` files — precision 9.0/27.0/73.8 s (min/mean/max), rapid 2.7/8.4/21.6 s, trace 48.8/65.1/81.7, survey 3.4/3.9/4.4 — and corrected the user guide's trace floor (48.8, not the 52 the visible table rows implied).
- **Root `README.md` rewritten.** It was still advertising "📋 Planning / proposal stage" with six dead doc links pointing at a pre-reorg `DOCS/` layout, and described the retired 8-gesture design. Now: problem statement with an honest-headline table, the mermaid architecture diagram, a component table, a **version-verified** tech stack (torch 2.10.0, timm 1.0.27, mediapipe 0.10.35, mlflow 3.10.1, optuna 4.8.0, onnxruntime 1.28.0, flask 3.1.0, Python 3.13.1), developer setup, run commands for pipeline / training / endpoint / real-time loop / UI, the required doc-name mapping, and a "things worth knowing before you trust a number here" section listing the five caveats (saturated benchmark, annotated-crop open-set numbers, head-init non-comparability, battery/clock, n=1 subject).
- **`plan.md`: Week-5 final status note** — MVP verdict against the three original proposal criteria (met / partially met / met), the measured-vs-played gap stated as the delivery's weakness, and **seven descoped items each with its reason**: landmark-hull retrain (re-bases every published number), instrumented live gameplay (cut for time — the wrong reason, said so), self-capture fine-tune, demo video, multi-subject usability, the `struct_unfreeze2_auglive` A/B, and re-running the open-set metric under live geometry.
- **AI docs closed out:** `AI-usage.md` Week-5 entry completed (crop-geometry investigation + AD-25 + the dashboard overhaul + this delivery work) plus a **final retrospective** on AI usage across the project — what worked (control runs that contradicted me, twice), what didn't (right digits with wrong meanings; a tendency to accept my framing when the framing was the problem; underweighting physical reality like the battery clock), how the approach evolved (from "generate this" to "prove this before I believe it"), and the effect on speed vs. quality. `Claude.md` finalized with a "How it actually went" section that records where usage **exceeded** the Week-0 plan (AI wrote most of the evidence-generation layer, not just boilerplate) and where ownership was weaker than the plan implies (infrastructure root causes were confirmed by reading evidence, not independently derived).
- **Deliverables written:** `week5/ui-walkthrough.md` (§1), `week5/how-to-use.md` (§2, non-technical), `week5/retrospective.md` (§5), `week5/README.md` (deliverable index + the assignment-name → repo-file mapping), `week5/ui-screenshots/` (12 PNGs).
- **No model retrained, no FSM value moved, no default flipped.** The last change to the control path remains AD-25's `pad` 0.15 → 0.35 (2026-07-28). Week 5 was UI polish, documentation, and the live run.
- **AI use:** Claude wrote the screenshot harness, found the CSS bug, drafted every Week-5 document, and rewrote the README, at Ted's request. The MVP verdict, the retrospective's framing (headline result = least-measured result), and the run-quality description are Ted's. Log in `AI-usage.md`.
- New: `scripts/capture_ui_screenshots.js`, `DOCS/class-related/week5/{README,ui-walkthrough,how-to-use,retrospective}.md`, `DOCS/class-related/week5/ui-screenshots/*.png`
- Changed: `README.md`, `DOCS/build/plan.md`, `DOCS/AI/Claude.md`, `DOCS/AI/AI-usage.md`, `src/eval_dashboard/static/style.css`
