...
> ⚠️ **The crop-source half of this contract was measured and was violated; the live `pad` default is now recalibrated** (2026-07-28, [AD-25](#ad-25--reconcile-the-live-crop-geometry-with-the-training-crop--accepted-2026-07-28)). Training crops HaGRID's **annotated bbox** at `pad = 0.15`; the live loop crops MediaPipe's **21-landmark hull**, now at `pad = 0.35` (was 0.15). A hull is not a bbox: it is **0.768×** the annotated box linearly (`fist` **0.705**, `palm` 0.849), so at the old pad the live crop was a ~1.3× zoom-in that hit the one class that fires clicks hardest. The `crop_mode` guard above checks *bbox-vs-full-frame*, not relative tightness, so it could not catch this. Quantified in [Strategy 3.2.4](strategies/3-cursor-and-click/03.2.4-crop-geometry-and-pad.md); reproduce with `scripts/eval_crop_geometry.py`.
...
### AD-14 — Drive the real game via `pydirectinput`; test registration early · *Accepted, amended by AD-17 (2026-07-14) and Strategy 3.2.3 (2026-07-27)*

> Amendment (3.2.3): `pydirectinput.click()` turned out to be the wrong primitive — it sends button-down and button-up in a **single** `SendInput` (0 ms hold), and a game that samples mouse state once per frame can miss it entirely. A click is now `mouseDown` → **~60 ms hold** → `mouseUp`, released via a per-frame deadline so the loop never blocks, with cursor motion suppressed for the duration of the press. `kill()` releases the button before disabling output. Rationale and measurements: [Strategy 3.2.3](strategies/3-cursor-and-click/03.2.3-latency-and-click-delivery.md).
...
### AD-20 — Single click per fist, edge-triggered · *Accepted, amended 2026-07-15 (Strategy 3.1.1), 2026-07-21 (Strategy 3.2.1)*

> Amendment (2026-07-21, Strategy 3.2.1): the FSM's `(conf, K)` starting points are now exposed as **named presets** in [`src/control/strategies.py`](../../src/control/strategies.py) — `3.2` (conf 0.70 / K 3, the original AD-20 values) and `3.2.1` (conf 0.80 / K 2, snappier). No FSM *logic* changes; the presets just tune the two knobs the decision always left to Ted, selectable via `play.py --strategy` and the eval dashboard's picker. See [Strategy 3.2.1](strategies/3-cursor-and-click/03.2.1-snappier-fsm.md).
...
## Tuning, orchestration & deployment (Week 4)

### AD-22 — Head initialization for a 2-class head · *Proposed (2026-07-26) — Ted's call*
- **Context:** Measured while building the Week-4 tuner. timm's EfficientNet-family `_init_weights` initializes a `Linear` head as `U(-r, r)` with **`r = 1/sqrt(out_features)`** — sized by the number of *classes*, not the 1280 input features. For ImageNet's 1000-class head that is r=0.032; for our **2-class** head it is **r=0.707**, ~25× PyTorch's own `Linear` default (1/sqrt(1280)=0.028). On 1280-d pooled features that starts the probe at logits of **±27** and cross-entropy **2.70** instead of ln(2)=0.69, so the first ~30 epochs are spent shrinking the init rather than learning. This is exactly the *"val still climbing at ep40"* note in the [fist-vs-rest record](../models/fistvsrest_frozen_mnv3_large/README.md).
- **Evidence:** same recipe, same seed, same cached features, only the head init changed: **val 0.9221 → 0.9740** (40 epochs; best epoch 38 → 16). Reproduced by `python -m src.tune --stage a`, which runs both inits as an explicit ablation and fails loudly if it can't reproduce the committed 0.9221.
- **Decision (proposed):** expose `head_init` on `build_model()` / the train config with **`timm_default` as the default** — so every committed run stays bit-reproducible — and use `pytorch_linear` for new runs. Whether `pytorch_linear` becomes the project-wide default (and whether the 8-class runs get re-baselined with it) is a **modeling decision left to Ted**; the mechanism and the measurement are in place either way.
- **Alternatives:** leave it alone and simply train longer (works, but wastes ~30 epochs and misreports "still climbing" as an epoch-budget problem); re-init the head by hand at each call site (same effect, no traceability); zero-init the head (converges but throws away symmetry breaking).
- **Consequences:** any run's init is now recorded in MLflow (`head_init` param) and in the checkpoint, so two runs' numbers are only comparable when it matches. If accepted as the default, the pre-pivot 8-class numbers in [DOCS/models/](../models/README.md) become "old-init" figures and should be labelled as such rather than silently compared against new ones.

### AD-23 — Orchestrate with a local task graph, not Airflow · *Accepted (2026-07-26)*
- **Context:** Week 4 asks for an orchestrated, resilient pipeline. The realistic constraints: one local Windows box (AD-02b), a **static** dataset, and a 2.5-hour CPU training path.
- **Decision:** a ~200-line DAG engine in [`src/pipeline/`](../../src/pipeline/dag.py) — topological ordering, per-task freshness (`cached`), retries, upstream-failure propagation, and a JSON run record per execution — with the expensive tasks run as **subprocesses** for isolation. Trigger is **event-based** (`--if-data-changed` compares the data version against the registered model's), explicitly **not** cron.
- **Rationale:** a scheduler daemon here would be infrastructure with no job to do: nothing arrives on a clock, so a nightly run would spend 2.5 h of CPU reproducing identical weights. What the project actually needs from orchestration is *ordering, idempotency, and an audit trail*, and those are the parts implemented. Keeping the task functions thin wrappers over existing modules means each step is still runnable by hand.
- **Alternatives:** Airflow / Prefect / Dagster (a scheduler, web server, and metadata DB to babysit for 9 tasks on one machine — rejected as ceremony, but the task boundaries are drawn so they'd port); a Makefile (freshness by file timestamp only — no run records, no retries); a shell script (no ordering guarantees, no audit).
- **Consequences:** no distributed execution and no backfill (there is no time dimension to backfill). Run records accumulate in `pipeline_runs/` (git-ignored; one representative record is committed with the Week-4 report). If this ever moves to a real scheduler, the port is per-task wrapping, not a redesign.

### AD-24 — MLflow Model Registry + a pyfunc artifact as the deployment contract · *Accepted (2026-07-26)*
- **Context:** Before Week 4, "the model" was a checkpoint path typed on a command line. Nothing recorded which checkpoint was current, and preprocessing lived in whatever code loaded it.
- **Decision:** register the model in the **MLflow Model Registry** (`fnaf-click-classifier`, `champion` alias) as a **pyfunc** artifact that carries its own preprocessing and returns a *click decision* (`predicted_class`, `confidence`, `p_click`, `click` at the AD-20 gate) — not raw logits. [`src/serve/app.py`](../../src/serve/app.py) serves whatever version holds the alias; [`src/serve/pyfunc_model.py`](../../src/serve/pyfunc_model.py) is the artifact.
- **Rationale:** preprocessing *is* half the contract (AD A.3) — putting it inside the versioned artifact makes a train/serve transform drift impossible for a served model. An alias move plus a restart is then the whole deployment, and "which model is live" has one answer.
- **Alternatives:** `mlflow.pytorch.log_model` (callers would have to re-implement the transform — the exact bug class AD A.3 exists to prevent); ONNX only (no metadata/versioning story, though ONNX **is** exported alongside with a parity check, AD-11); keep passing checkpoint paths (no versioning, no provenance).
- **Consequences:** the file-based registry (`./mlruns`) prints a deprecation warning and would need a DB backend for real multi-user use. **The HTTP endpoint is not in the real-time loop**: ~35-40 ms of model time on this CPU against a ~33 ms frame budget means the game loop calls the model in-process, and the endpoint exists for validation, batch scoring, and a versioned network surface.

## Crop geometry (Week 5)

### AD-25 — Reconcile the live crop geometry with the training crop · *Accepted (2026-07-28)*
- **Context:** A live report that `not_fist` reads >90% while a real `fist` "caps around 55%" — under both FSM gates (0.70 / 0.80), so those fists can never click. The hypothesis was overfitting; the measurement says otherwise. Architecture §A.3 requires the live detector's box to match training's in **relative tightness**, and both halves do call the same `padded_bbox_pixels()` with the same `pad = 0.15` — but on **different boxes**: training pads HaGRID's *annotated hand bbox*, the live loop pads MediaPipe's *21-landmark hull*. The clause has been asserted since Strategy 2 and flagged "unmeasured" in every model record since.
- **Evidence** (`scripts/eval_crop_geometry.py`, both stages):
  - **Geometry, full population** (HaGRID's annotations carry the bbox *and* the 21 landmarks, so the hull is recoverable offline; 27k–29k images/gesture): hull ÷ annotated box = **0.768× linear** overall, and the spread is anatomical — **`fist` 0.705** (curled fingers sit inside the silhouette) up to `palm` 0.849 (landmarks reach the fingertips). The live crop is a ~1.3× zoom-in, worst on the one class that fires clicks.
  - **MediaPipe is not the cause.** Its detected hull vs. HaGRID's *ground-truth* landmark hull on the same images: **0.995–1.054×** across all seven gestures. The gap is a hull-vs-bbox **definition** mismatch, not detection error.
  - **Cost at the click gate** (held-out test images, same checkpoint, annotated crop vs. real MediaPipe crop, n = 54 fists / 140 non-fists, gate 0.70): at `pad 0.15` fist click-rate **81.5%** and 10th-percentile p(fist) **0.464**; at `pad 0.35`, **94.4%** and **0.932**, with false clicks **0.7% → 0.0%**. The *median* barely moves (0.927 → 0.984) — a good average was hiding the tail, and `K` consecutive frames over the gate is precisely what a bottom-decile frame breaks.
  - **Not overfitting:** val = test = 1.0000 with no variance gap (leakage-audited in 3.2.2), the degradation *reverses* when only the crop changes, and the tuned champion is the **most** robust of the five checkpoints under live geometry (81.5% / 0.7% false vs. the 3.2 model it replaced at 77.8% / **10.7%**). ONNX (2.3e-07 max probability disagreement on real crops) and head bias (per-class weight norms 0.598 / 0.602) were both cleared.
- **Decision:** **(a) accepted, 2026-07-28 (Ted's call).** The live `pad` default is recalibrated **0.15 → 0.35** in `play.py`, `webcam_demo.py`, and the dashboard tracker (`src/eval_dashboard/tracker.py`) — one flag, instantly revertible (`--pad 0.15`), leaves the per-gesture spread unfixed (equalizing pad is still 0.42 for `fist` vs. 0.27 for `palm`). Training itself is untouched: `configs/data_fistvsrest.yaml`'s `bbox_pad` stays at 0.15, since that pads the *annotated* box, not the hull. **(b) retrain on landmark-hull crops** remains open — HaGRID's landmarks are already on disk, so training could build its crop the same way the live detector does, making the live box a **1.00×** match by construction. (b) costs a data-layer option (`crop_source: bbox | landmark_hull`) plus a ~2.5 h run, and re-bases every published accuracy number onto a different crop; it is not superseded by (a), just not taken yet. **Still untested:** the live A/B against the actual game — this measurement is on HaGRID stills (a lower bound, see Consequences).
- **Alternatives:** a per-class pad (the live loop doesn't know the class before it classifies — circular); square-padding the hull to the training aspect ratio (addresses shape, not scale); leaving it and lowering the FSM gate (trades false clicks for true ones, and 3.2 exists to *avoid* that trade).
- **Consequences:** every false-click number this project has published — including 3.2's headline unseen-`ok` 0.8% — was measured on **annotated** crops, so all of them are optimistic by an unmeasured amount under live geometry. This also bounds the claim: the measurement is on HaGRID stills, so it isolates *geometry* and is a **lower bound** on the live gap; webcam sensor, lighting, and the arm's-length pose regime HaGRID lacks are still unmeasured on top. Full write-up: [Strategy 3.2.4](strategies/3-cursor-and-click/03.2.4-crop-geometry-and-pad.md).
...
- ~~**Latency budget:** does MediaPipe + cursor mapper + click FSM fit ≥15 FPS end-to-end (carried from AD-04; measure on day 1 of runtime work)?~~ **Answered 2026-07-27 (Strategy 3.2.3):** yes, with margin — measured end-to-end at **28.3 FPS** (32.6 ms/frame: MediaPipe 19.2, classifier 6.3 on ONNX, preprocess 2.6, HUD 2.7), against a 30 FPS camera that is now the binding constraint. On the PyTorch backend it was 21.0 FPS, still above the ≥15 bar; with torch's *default* thread pool it fell to 14.7 FPS, i.e. the budget was only ever missed by accident. Re-measure with [`scripts/bench_pipeline.py`](../../scripts/bench_pipeline.py) after any loop change. **Open sub-question this leaves:** whether ONNX-in-the-loop and the 60 ms click hold deserve their own decision record, or stay documented in [3.2.3](strategies/3-cursor-and-click/03.2.3-latency-and-click-delivery.md) as amendments to AD-11 and AD-14 — Ted's call.
- **AD-11** TorchScript vs. ONNX — pick whichever hits the latency budget with clean parity.
- Whether a **small self-captured fine-tune** (my hands, my room) is needed to close the train/serve gap — more likely *yes* now, since cursor use lives exactly in the arm-extended regime HaGRID lacks. **Note (2026-07-28):** part of what looked like this gap turned out to be plain geometry — see AD-25 — so measure the pad fix *first*; whatever survives it is the genuine domain gap a self-capture set would have to close.
- **AD-25 residual:** the live `pad` default moved 0.15 → 0.35 (accepted); whether to also retrain on landmark-hull crops is still open, and this measurement's fist click-rate (81.5% → 94.4%) and false-click (0.7% → 0.0%) numbers are on HaGRID stills, not the live game — that A/B is still pending, same as every strategy since 3.1. The open-set false-click numbers should also be re-measured under live geometry — every one published so far used annotated crops.
...
| 2026-07-26 | **Week 4 — AD-22 proposed, AD-23 + AD-24 accepted.** AD-22: timm's 2-class head init (`r=1/sqrt(out_features)` → ±27 logits) measured as worth **+5.2 val points** on its own; `head_init` knob added with `timm_default` preserved as the default (adopting `pytorch_linear` project-wide is Ted's call). AD-23: local task-graph orchestration in `src/pipeline/` with event-based (not scheduled) triggering. AD-24: MLflow Model Registry + a pyfunc artifact that carries its own preprocessing as the deployment contract; ONNX exported alongside with a parity check (AD-11). Full write-up: [Week-4 report](../class-related/week4/tuning-orchestration-report.md). |
| 2026-07-21 | **Strategy 3.2.1 — AD-20 amended (FSM presets)**: the click FSM's `(conf, K)` tuning is surfaced as named presets in `src/control/strategies.py` — `3.2` (0.70 / 3) and `3.2.1` (0.80 / 2, snappier). No FSM logic change; same 3.2 checkpoint. Selectable via `play.py --strategy` and an in-page **Click-strategy picker** on the eval dashboard (fist-vs-rest checkpoints only), which stamps the chosen strategy onto each saved run. Live 3.2-vs-3.2.1 A/B pending. |
| 2026-07-27 | **Strategy 3.2.3 — runtime, not model.** The live loop's frame budget was measured end-to-end for the first time (`scripts/bench_pipeline.py`): **21.0 → 28.3 FPS** (45.7 → 32.6 ms/frame) by serving stage 2 from the already-exported, parity-checked `model.onnx` (AD-11: classifier 20.7 → 6.2 ms) and capping torch's thread pool at 4 (its 16-thread default starves MediaPipe *and* torchvision preprocessing — 14.7 FPS otherwise). Threaded newest-frame-only capture removes a measured **2-frame / ~67 ms** camera backlog (input lag, not throughput). **AD-14 amended:** a click is now `mouseDown` → **~60 ms hold** → `mouseUp` released without blocking the loop, with the cursor frozen while pressed — `pydirectinput.click()` sends down+up in one `SendInput` (0 ms hold), which a 60 Hz DirectX title can sample right past. Kill-switch contract extended: `kill()` releases the button first. **No FSM value changed** (AD-20 untouched). The AD-04 latency question is closed; whether any of this warrants its own AD is Ted's call. |
| 2026-07-28 | **AD-25 proposed then accepted — Strategy 3.2.4, the crop-source half of the §A.3 contract measured and corrected.** Training pads HaGRID's *annotated bbox*; the live loop pads MediaPipe's *21-landmark hull* — the same `pad = 0.15` on two different boxes. Measured over the full annotation population: the hull is **0.768×** the annotated box linearly, `fist` worst at **0.705** (curled fingers sit inside the silhouette) vs. `palm` 0.849, so the live crop was a ~1.3× zoom-in on the one class that fires clicks. MediaPipe exonerated (its hull matches HaGRID's ground-truth hull at 0.995–1.054×) — the gap is a hull-vs-bbox *definition* mismatch. At the FSM gate this cost the tail, not the average: at `pad 0.15` a real fist's **p10 confidence was 0.464** (under both gates) and click-rate 81.5%; at `pad 0.35`, **0.932** and 94.4%, with false clicks 0.7% → 0.0%. Overfitting ruled out (no variance gap; the loss reverses when only the crop changes; the tuned champion is the *most* robust of five under live geometry). **Ted accepted the recalibration**: `--pad` default moved 0.15 → 0.35 in `play.py`, `webcam_demo.py`, and the dashboard tracker (training's `bbox_pad` unchanged); retraining on landmark-hull crops remains open, and the live A/B against the real game is still pending. New: `scripts/eval_crop_geometry.py`. |
